...
for j in range(8):
    ...
    flow[k:k+30] -= rng.uniform(800,1200)  # dips
    vib[k:k+10] += rng.uniform(1.0,2.0)
df = pd.DataFrame({'ts':idx, 'flow_kg_s':flow, 'dp_kPa':dp, 'temp_C':temp, 'vib_mm_s':vib})
df.to_csv(base/'mock_sensors.csv', index=False)

# Events (failures and WOs)
events = []
for i in range(10):
    t0 = start + dt.timedelta(hours=random.uniform(0, 7*24-4))
    t1 = t0 + dt.timedelta(hours=random.uniform(1, 4))
    events.append({'type': random.choice(['failure','wo_open','wo_close']),
                   'start': t0.isoformat(), 'end': t1.isoformat(), 'system':'ServiceWater'})
pd.DataFrame(events).to_csv(base/'events.csv', index=False)
print('Generated data in data/*.csv')
